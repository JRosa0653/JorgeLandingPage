...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    // Validate stored session token + expiry
    const token = sessionStorage.getItem("jorge_admin_token");
    const expiry = sessionStorage.getItem("jorge_admin_expiry");
    if (!token || !expiry) return false;
    if (Date.now() > parseInt(expiry, 10)) {
      sessionStorage.removeItem("jorge_admin_token");
      sessionStorage.removeItem("jorge_admin_expiry");
      return false;
    }
    return true;
  });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        sessionStorage.setItem("jorge_admin_token", data.token);
        sessionStorage.setItem("jorge_admin_expiry", String(data.expiresAt));
    ...
    return (
      <div id="admin-auth-panel" className="max-w-md mx-auto my-12 p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Administrator Passcode</label>
            <input
          ...
          <button
            ...
            type="submit"
            disabled={isLoading}
            className="w-full py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow-lg active:scale-95"
          >
            {isLoading ? "Verificando..." : "Unlock Dashboard"}
          </button>
        </form>
        ...
        <div className="text-center mt-6 text-[10px] text-slate-500 font-semibold uppercase tracking-wider bg-slate-950/40 py-2.5 rounded-lg border border-slate-800/40">
          🔒 Authorized personnel only · Sessions expire after 4 hours
        </div>
      </div>
    );
  }
